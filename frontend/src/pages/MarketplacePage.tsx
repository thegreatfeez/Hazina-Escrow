...
import { Search, SlidersHorizontal, TrendingUp, Clock, DollarSign, X } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen pt-28 pb-20">
      <div className="max-w-7xl mx-auto px-4">
        {/* Header */}
        <div className="mb-10">
          <p className="text-gold text-sm font-body font-medium tracking-widest uppercase mb-2">Browse & Buy</p>
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-foreground-muted font-body">
            {loading ? 'Loading...' : (
              <><span className="text-foreground font-medium">{filtered.length}</span> datasets found</>
            )}
          </p>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="glass-card p-6 animate-pulse">
                <div className="h-4 bg-surface-2 rounded mb-3 w-20" />
                <div className="h-6 bg-surface-2 rounded mb-2 w-full" />
                <div className="h-4 bg-surface-2 rounded mb-1 w-full" />
                <div className="h-4 bg-surface-2 rounded mb-6 w-3/4" />
          ...
          <div className="text-center py-24">
            ...
            <h3 className="font-display text-xl text-foreground mb-2">No datasets found</h3>
            <p className="text-foreground-muted font-body text-sm">Try adjusting your filters</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((ds) => (
